...
for (let i = 0; i < picture.length; i++) {
  for (let j = 0; j < suit.length; j++) {
    let cardInfo = `<div class="card__info">${picture[
      i
    ][0].toUpperCase()}<img src="images/${suit[j]}.svg" alt="${
      suit[j]
    }"></div>`;
    let centerImg = picture[i] !== "t" ? picture[i] : suit[j];
    cards.push(
      `<div class="card card--person">${cardInfo}<img class="person" src="images/${centerImg}.svg" alt="${centerImg}">${cardInfo}</div>`
    );
  }
}
